/*============================================================================
 *  DSOX2012APlugin.cpp - Keysight InfiniiVision 2000 X-Series plugin.
 ...
 *  MockVisa emulator; TODO(manual) constants await manuals/DSOX2012A/.
 *==========================================================================*/
#include "DSOX2012APlugin.h"
...
#define KS_MODEL_NAME "DSOX2012A"

CDSOX2012APlugin::CDSOX2012APlugin()
    : m_pLimits(nullptr)
...
S_Scope_PluginInfo CDSOX2012APlugin::getPluginInfo() const
{
...
S_Scope_Capabilities CDSOX2012APlugin::getCapabilities() const
{
...
CDSOX2012APlugin::S_DeviceInstance* CDSOX2012APlugin::dev(U32BIT s)
{ auto it = m_devices.find(s); return (it == m_devices.end()) ? nullptr : &it.value(); }
bool CDSOX2012APlugin::validChannel(U32BIT c) const
{ return c >= 1 && static_cast<int>(c) <= m_pLimits->m_iAnalogChannels; }
QByteArray CDSOX2012APlugin::fmtD(FDOUBLE v)
{ char sz[40]; std::snprintf(sz, sizeof(sz), "%.6G", v); return QByteArray(sz); }
int CDSOX2012APlugin::chanOf(Enum_Scope_TriggerSource e)
{
    switch (e) {
...
ScopeError CDSOX2012APlugin::visaError(ViStatus st, const QString& ctx)
{
    if (st == VI_ERROR_TMO)
...
ScopeError CDSOX2012APlugin::setDouble(U32BIT s, const char* scpi, FDOUBLE v, FDOUBLE lo, FDOUBLE hi, const char* what)
{
...
ScopeError CDSOX2012APlugin::disconnect(U32BIT s)
{
    S_DeviceInstance* d = dev(s);
...
bool CDSOX2012APlugin::isConnected(U32BIT s) const
{ auto it = m_devices.find(s); return it != m_devices.end() && it.value().m_bConnected; }
ScopeError CDSOX2012APlugin::reset(U32BIT s)       { return sendChecked(s, QByteArrayLiteral("*RST")); }
ScopeError CDSOX2012APlugin::clearStatus(U32BIT s) { return writeLine(s, QByteArrayLiteral("*CLS")); }
ScopeError CDSOX2012APlugin::setTimeout(U32BIT s, U32BIT t)
...
ScopeError CDSOX2012APlugin::getIdentification(U32BIT s, QString& o)
{ QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral("*IDN?"), r); if (e.isSuccess()) o = QString::fromLatin1(r).trimmed(); return e; }
ScopeError CDSOX2012APlugin::selfTest(U32BIT s, S32BIT& o)
{ FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("*TST?"), d); if (e.isSuccess()) o = static_cast<S32BIT>(d); return e; }
ScopeError CDSOX2012APlugin::getOptions(U32BIT s, QString& o)
{ QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral("*OPT?"), r); if (e.isSuccess()) o = QString::fromLatin1(r).trimmed(); return e; }
ScopeError CDSOX2012APlugin::waitOperationComplete(U32BIT s) { QByteArray r; return queryLine(s, QByteArrayLiteral("*OPC?"), r); }
ScopeError CDSOX2012APlugin::getScpiVersion(U32BIT s, QString& o)
{ if (!isConnected(s)) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = QStringLiteral("1999.0"); return ScopeError(); }

ScopeError CDSOX2012APlugin::getParameterRange(U32BIT s, U32BIT c, Enum_Scope_ParamId p, S_Scope_ParameterRange& o)
{
...
ScopeError CDSOX2012APlugin::autoscale(U32BIT s)    { return sendChecked(s, QByteArrayLiteral(":AUToscale")); }
ScopeError CDSOX2012APlugin::run(U32BIT s)          { ScopeError e = writeLine(s, QByteArrayLiteral(":RUN")); if (e.isSuccess()) { S_DeviceInstance* d = dev(s); if (d) d->m_bRunning = true; } return e; }
ScopeError CDSOX2012APlugin::stop(U32BIT s)         { ScopeError e = writeLine(s, QByteArrayLiteral(":STOP")); if (e.isSuccess()) { S_DeviceInstance* d = dev(s); if (d) d->m_bRunning = false; } return e; }
ScopeError CDSOX2012APlugin::single(U32BIT s)       { ScopeError e = writeLine(s, QByteArrayLiteral(":SINGle")); if (e.isSuccess()) { S_DeviceInstance* d = dev(s); if (d) d->m_bRunning = false; } return e; }
ScopeError CDSOX2012APlugin::forceTrigger(U32BIT s) { return writeLine(s, QByteArrayLiteral(":TRIGger:FORCe")); }

ScopeError CDSOX2012APlugin::enableChannel(U32BIT s, U32BIT c, bool v)
...
ScopeError CDSOX2012APlugin::isChannelEnabled(U32BIT s, U32BIT c, bool& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArray(":CHANnel") + QByteArray::number(c) + ":DISPlay?", d); if (e.isSuccess()) o = (d != 0.0); return e; }
ScopeError CDSOX2012APlugin::setVerticalScale(U32BIT s, U32BIT c, FDOUBLE v)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CDSOX2012APlugin::getVerticalScale(U32BIT s, U32BIT c, FDOUBLE& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return queryDouble(s, QByteArray(":CHANnel") + QByteArray::number(c) + ":SCALe?", o); }
ScopeError CDSOX2012APlugin::setVerticalOffset(U32BIT s, U32BIT c, FDOUBLE v)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CDSOX2012APlugin::getVerticalOffset(U32BIT s, U32BIT c, FDOUBLE& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return queryDouble(s, QByteArray(":CHANnel") + QByteArray::number(c) + ":OFFSet?", o); }
ScopeError CDSOX2012APlugin::setCoupling(U32BIT s, U32BIT c, Enum_Scope_Coupling v)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CDSOX2012APlugin::getCoupling(U32BIT s, U32BIT c, Enum_Scope_Coupling& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CDSOX2012APlugin::setProbeAttenuation(U32BIT s, U32BIT c, FDOUBLE v)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CDSOX2012APlugin::getProbeAttenuation(U32BIT s, U32BIT c, FDOUBLE& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return queryDouble(s, QByteArray(":CHANnel") + QByteArray::number(c) + ":PROBe?", o); }

ScopeError CDSOX2012APlugin::setTimebaseScale(U32BIT s, FDOUBLE v)
{ return setDouble(s, ":TIMebase:SCALe", v, m_pLimits->m_dTimebaseMin, m_pLimits->m_dTimebaseMax, "timebase scale"); }
ScopeError CDSOX2012APlugin::getTimebaseScale(U32BIT s, FDOUBLE& o) { return queryDouble(s, QByteArrayLiteral(":TIMebase:SCALe?"), o); }
ScopeError CDSOX2012APlugin::setTimebasePosition(U32BIT s, FDOUBLE v) { return sendChecked(s, QByteArray(":TIMebase:POSition ") + fmtD(v)); }
ScopeError CDSOX2012APlugin::getTimebasePosition(U32BIT s, FDOUBLE& o) { return queryDouble(s, QByteArrayLiteral(":TIMebase:POSition?"), o); }
ScopeError CDSOX2012APlugin::getSampleRate(U32BIT s, FDOUBLE& o)
{
...
ScopeError CDSOX2012APlugin::setMemoryDepth(U32BIT s, U32BIT v)
...
ScopeError CDSOX2012APlugin::getMemoryDepth(U32BIT s, U32BIT& o)
...
ScopeError CDSOX2012APlugin::setTriggerMode(U32BIT s, Enum_Scope_TriggerMode v)
...
ScopeError CDSOX2012APlugin::getTriggerMode(U32BIT s, Enum_Scope_TriggerMode& o)
...
ScopeError CDSOX2012APlugin::setTriggerSource(U32BIT s, Enum_Scope_TriggerSource v)
...
ScopeError CDSOX2012APlugin::getTriggerSource(U32BIT s, Enum_Scope_TriggerSource& o)
{ QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral(":TRIGger:EDGE:SOURce?"), r); if (!e.isSuccess()) return e;
  ...
  return ScopeError(); }
ScopeError CDSOX2012APlugin::setTriggerSlope(U32BIT s, Enum_Scope_TriggerSlope v)
...
ScopeError CDSOX2012APlugin::getTriggerSlope(U32BIT s, Enum_Scope_TriggerSlope& o)
{ QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral(":TRIGger:EDGE:SLOPe?"), r); if (!e.isSuccess()) return e;
  ...
  return ScopeError(); }
ScopeError CDSOX2012APlugin::setTriggerLevel(U32BIT s, U32BIT c, FDOUBLE v)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":TRIGger:EDGE:LEVel ") + fmtD(v) + ",CHANnel" + QByteArray::number(c)); }
ScopeError CDSOX2012APlugin::getTriggerLevel(U32BIT s, U32BIT c, FDOUBLE& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return queryDouble(s, QByteArrayLiteral(":TRIGger:EDGE:LEVel?"), o); }
ScopeError CDSOX2012APlugin::setTriggerHoldoff(U32BIT s, FDOUBLE v)
{ return setDouble(s, ":TRIGger:HOLDoff", v, m_pLimits->m_dTrigHoldoffMin, m_pLimits->m_dTrigHoldoffMax, "holdoff"); }
ScopeError CDSOX2012APlugin::getTriggerHoldoff(U32BIT s, FDOUBLE& o) { return queryDouble(s, QByteArrayLiteral(":TRIGger:HOLDoff?"), o); }
ScopeError CDSOX2012APlugin::getTriggerState(U32BIT s, Enum_Scope_TriggerState& o)
...
ScopeError CDSOX2012APlugin::setAcqMode(U32BIT s, Enum_Scope_AcqMode v)
...
ScopeError CDSOX2012APlugin::getAcqMode(U32BIT s, Enum_Scope_AcqMode& o)
{ QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral(":ACQuire:TYPE?"), r); if (!e.isSuccess()) return e;
  ...
  return ScopeError(); }
ScopeError CDSOX2012APlugin::setAverageCount(U32BIT s, U32BIT v)
...
ScopeError CDSOX2012APlugin::getAverageCount(U32BIT s, U32BIT& o)
{ FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral(":ACQuire:COUNt?"), d); if (e.isSuccess()) o = static_cast<U32BIT>(d); return e; }
ScopeError CDSOX2012APlugin::getAcquisitionState(U32BIT s, Enum_Scope_AcqState& o)
...
ScopeError CDSOX2012APlugin::setWaveformSource(U32BIT s, Enum_Scope_WaveformSource v, U32BIT i)
...
ScopeError CDSOX2012APlugin::setWaveformFormat(U32BIT s, Enum_Scope_WaveformFormat v)
...
ScopeError CDSOX2012APlugin::setWaveformPoints(U32BIT s, U32BIT v)
...
ScopeError CDSOX2012APlugin::getWaveformPreamble(U32BIT s, U32BIT c, S_Scope_WaveformPreamble& o)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  ...
  return ScopeError(); }
ScopeError CDSOX2012APlugin::digitizeChannel(U32BIT s, U32BIT c)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":DIGitize CHANnel") + QByteArray::number(c)); }

ScopeError CDSOX2012APlugin::captureScreenshot(U32BIT s, Enum_Scope_ImageFormat f, QByteArray& o)
{
...
ScopeError CDSOX2012APlugin::readErrorStatus(U32BIT s, U32BIT, S_Scope_DeviceErrorStatus& o)
...
ScopeError CDSOX2012APlugin::clearErrorStatus(U32BIT s, U32BIT) { return writeLine(s, QByteArrayLiteral("*CLS")); }
ScopeError CDSOX2012APlugin::queryErrorQueue(U32BIT s, QString& o)
...
ScopeError CDSOX2012APlugin::readStatusByte(U32BIT s, U32BIT& o)
{ FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("*STB?"), d); if (e.isSuccess()) o = static_cast<U32BIT>(d); return e; }
ScopeError CDSOX2012APlugin::readStandardEventStatus(U32BIT s, U32BIT& o)
{ FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("*ESR?"), d); if (e.isSuccess()) o = static_cast<U32BIT>(d); return e; }
ScopeError CDSOX2012APlugin::writeScpi(U32BIT s, const QString& v) { return writeLine(s, v.toLatin1()); }
ScopeError CDSOX2012APlugin::queryScpi(U32BIT s, const QString& v, QString& o)
...
ScopeError CDSOX2012APlugin::addMeasurement(U32BIT s, U32BIT c, Enum_Scope_MeasType t)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":MEASure:") + ksMeasTok(t) + " CHANnel" + QByteArray::number(c)); }
ScopeError CDSOX2012APlugin::readMeasurement(U32BIT s, U32BIT c, Enum_Scope_MeasType t, S_Scope_MeasurementResult& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    FDOUBLE v = 0.0;
...
ScopeError CDSOX2012APlugin::clearMeasurements(U32BIT s) { return writeLine(s, QByteArrayLiteral(":MEASure:CLEar")); }
ScopeError CDSOX2012APlugin::setMeasureStatistics(U32BIT s, bool v)
{ return sendChecked(s, QByteArray(":MEASure:STATistics ") + (v ? "ON" : "OFF")); }
ScopeError CDSOX2012APlugin::getMeasurementStatistics(U32BIT s, U32BIT c, Enum_Scope_MeasType t, S_Scope_MeasurementResult& o)
{ return readMeasurement(s, c, t, o); }

ScopeError CDSOX2012APlugin::setCursorType(U32BIT s, Enum_Scope_CursorType v)
{
...
ScopeError CDSOX2012APlugin::getCursorType(U32BIT s, Enum_Scope_CursorType& o)
{
...
ScopeError CDSOX2012APlugin::setCursorSource(U32BIT s, U32BIT c)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":MARKer:X1Y1source CHANnel") + QByteArray::number(c)); }
ScopeError CDSOX2012APlugin::setCursorPosition(U32BIT s, U32BIT i, FDOUBLE v)
{ return sendChecked(s, QByteArray(":MARKer:X") + QByteArray::number(i == 0 ? 1 : i) + "Position " + fmtD(v)); }
ScopeError CDSOX2012APlugin::getCursorPosition(U32BIT s, U32BIT i, FDOUBLE& o)
{ return queryDouble(s, QByteArray(":MARKer:X") + QByteArray::number(i == 0 ? 1 : i) + "Position?", o); }
ScopeError CDSOX2012APlugin::readCursorValues(U32BIT s, FDOUBLE& x1, FDOUBLE& x2, FDOUBLE& y1, FDOUBLE& y2)
{
...
ScopeError CDSOX2012APlugin::setMathOperation(U32BIT s, Enum_Scope_MathOp v)
{
    const char* op = "ADD";
    switch (v) {
...
ScopeError CDSOX2012APlugin::setMathSource1(U32BIT s, U32BIT c)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":FUNCtion:SOURce1 CHANnel") + QByteArray::number(c)); }
ScopeError CDSOX2012APlugin::setMathSource2(U32BIT s, U32BIT c)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":FUNCtion:SOURce2 CHANnel") + QByteArray::number(c)); }
ScopeError CDSOX2012APlugin::enableMath(U32BIT s, bool v) { return sendChecked(s, QByteArray(":FUNCtion:DISPlay ") + (v ? "ON" : "OFF")); }
ScopeError CDSOX2012APlugin::setFftWindow(U32BIT s, Enum_Scope_FftWindow v)
{
...
ScopeError CDSOX2012APlugin::getFftWindow(U32BIT s, Enum_Scope_FftWindow& o)
{
...
ScopeError CDSOX2012APlugin::setFftSpan(U32BIT s, FDOUBLE v)   { return sendChecked(s, QByteArray(":FUNCtion:FFT:SPAN ") + fmtD(v)); }
ScopeError CDSOX2012APlugin::setFftCenter(U32BIT s, FDOUBLE v) { return sendChecked(s, QByteArray(":FUNCtion:FFT:CENTer ") + fmtD(v)); }
ScopeError CDSOX2012APlugin::setMathScale(U32BIT s, FDOUBLE v)    { return sendChecked(s, QByteArray(":FUNCtion:SCALe ") + fmtD(v)); }
ScopeError CDSOX2012APlugin::setMathPosition(U32BIT s, FDOUBLE v) { return sendChecked(s, QByteArray(":FUNCtion:OFFSet ") + fmtD(v)); }

ScopeError CDSOX2012APlugin::setPersistence(U32BIT s, FDOUBLE v)
{ return sendChecked(s, QByteArray(":DISPlay:PERSistence ") + ((v > 0.0) ? fmtD(v) : QByteArray("MINimum"))); }
ScopeError CDSOX2012APlugin::setGraticule(U32BIT s, const QString& v) { return sendChecked(s, QByteArray(":DISPlay:GRATicule ") + v.toLatin1()); }
ScopeError CDSOX2012APlugin::setIntensity(U32BIT s, FDOUBLE v) { return sendChecked(s, QByteArray(":DISPlay:INTensity:WAVeform ") + fmtD(v)); }
ScopeError CDSOX2012APlugin::setDisplayFormat(U32BIT s, Enum_Scope_TimebaseMode v)
{ return sendChecked(s, QByteArray(":TIMebase:MODE ") + ((v == Enum_Scope_TimebaseMode::m_enumXY) ? "XY" : "MAIN")); }
ScopeError CDSOX2012APlugin::setVectors(U32BIT s, bool v) { return sendChecked(s, QByteArray(":DISPlay:VECTors ") + (v ? "ON" : "OFF")); }

ScopeError CDSOX2012APlugin::saveSetup(U32BIT s, U32BIT loc)  { return sendChecked(s, QByteArray("*SAV ") + QByteArray::number(loc)); }
ScopeError CDSOX2012APlugin::recallSetup(U32BIT s, U32BIT loc){ return sendChecked(s, QByteArray("*RCL ") + QByteArray::number(loc)); }
ScopeError CDSOX2012APlugin::saveWaveformToFile(U32BIT s, U32BIT c, const QString& path)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
  return sendChecked(s, QByteArray(":SAVE:WAVeform:STARt \"") + path.toLatin1() + "\""); }
ScopeError CDSOX2012APlugin::saveToReference(U32BIT s, U32BIT c, U32BIT slot)
{ if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CDSOX2012APlugin::displayReference(U32BIT s, U32BIT slot, bool v)
{ (void)slot; return sendChecked(s, QByteArray(":REFerence:DISPlay ") + (v ? "ON" : "OFF")); }

ScopeError CDSOX2012APlugin::enableDigitalChannel(U32BIT s, U32BIT d, bool v)
...
ScopeError CDSOX2012APlugin::setDigitalThreshold(U32BIT s, U32BIT d, FDOUBLE v)
{ if (!m_pLimits->m_bHasDigital) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
  return sendChecked(s, QByteArray(":DIGital") + QByteArray::number(d) + ":THReshold " + fmtD(v)); }
ScopeError CDSOX2012APlugin::setPodThreshold(U32BIT s, U32BIT p, FDOUBLE v)
{ if (!m_pLimits->m_bHasDigital) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
  return sendChecked(s, QByteArray(":POD") + QByteArray::number(p) + ":THReshold " + fmtD(v)); }
ScopeError CDSOX2012APlugin::enableBus(U32BIT s, U32BIT b, bool v)
...
ScopeError CDSOX2012APlugin::setBusType(U32BIT s, U32BIT b, const QString& v)
{ if (!m_pLimits->m_bHasSerialDecode) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
  return sendChecked(s, QByteArray(":SBUS") + QByteArray::number(b) + ":MODE " + v.toLatin1()); }
ScopeError CDSOX2012APlugin::readBusDecode(U32BIT s, U32BIT b, QString& o)
{ if (!m_pLimits->m_bHasSerialDecode) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
...
ScopeError CDSOX2012APlugin::setAwgFunction(U32BIT s, const QString& v)
...
ScopeError CDSOX2012APlugin::setAwgFrequency(U32BIT s, FDOUBLE v)
{ if (!m_pLimits->m_bHasAWG) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
...
ScopeError CDSOX2012APlugin::setAwgAmplitude(U32BIT s, FDOUBLE v)
{ if (!m_pLimits->m_bHasAWG) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
...
ScopeError CDSOX2012APlugin::setAwgOffset(U32BIT s, FDOUBLE v)
{ if (!m_pLimits->m_bHasAWG) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
  return sendChecked(s, QByteArray(":WGEN:VOLTage:OFFSet ") + fmtD(v)); }
ScopeError CDSOX2012APlugin::enableAwgOutput(U32BIT s, bool v)
{ if (!m_pLimits->m_bHasAWG) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
  return sendChecked(s, QByteArray(":WGEN:OUTPut ") + (v ? "ON" : "OFF")); }

ScopeError CDSOX2012APlugin::readOperationStatus(U32BIT s, U32BIT& o)
...
ScopeError CDSOX2012APlugin::readQuestionableStatus(U32BIT s, U32BIT& o)
{ (void)s; o = 0u; return ScopeError(); }
ScopeError CDSOX2012APlugin::getInstrumentErrorCount(U32BIT s, U32BIT& o)
...
ScopeError CDSOX2012APlugin::setRemoteState(U32BIT s, Enum_Scope_RemoteState v)
{ (void)v; return writeLine(s, QByteArrayLiteral("*OPC")); }
ScopeError CDSOX2012APlugin::getRemoteState(U32BIT s, Enum_Scope_RemoteState& o)
{ (void)s; o = Enum_Scope_RemoteState::m_enumRemote; return ScopeError(); }
ScopeError CDSOX2012APlugin::setKeyLock(U32BIT s, bool v) { return sendChecked(s, QByteArray(":SYSTem:LOCK ") + (v ? "ON" : "OFF")); }
ScopeError CDSOX2012APlugin::isKeyLocked(U32BIT s, bool& o)
{ FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral(":SYSTem:LOCK?"), d); if (e.isSuccess()) o = (d != 0.0); return e; }
ScopeError CDSOX2012APlugin::setBeeper(U32BIT s, bool v) { (void)v; return writeLine(s, QByteArrayLiteral("*CLS")); }
