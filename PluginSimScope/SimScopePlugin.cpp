...
{
    switch (in_eShape) {
    case 0: /* SINE   */ return std::sin(2.0 * PI * in_dPhase);
    case 1: /* SQUARE */ return (in_dPhase < 0.5) ? 1.0 : -1.0;
    case 2: /* RAMP   */ return 2.0 * in_dPhase - 1.0;
    case 3: /* NOISE  */ {
...
CSimScopePlugin::~CSimScopePlugin() { m_devices.clear(); }
...
{
    return d != nullptr && d->m_pLimits != nullptr &&
           c >= 1 && static_cast<int>(c) <= d->m_pLimits->m_iAnalogChannels;
}
...
ScopeError CSimScopePlugin::connect(U32BIT s, const S_Scope_ConnectionConfig& c)
{
    ...
    if (strRes.contains(QStringLiteral("SQUARE"))) d.m_eShape = SHAPE_SQUARE;
    else if (strRes.contains(QStringLiteral("RAMP"))) d.m_eShape = SHAPE_RAMP;
    else if (strRes.contains(QStringLiteral("NOISE"))) d.m_eShape = SHAPE_NOISE;
...
ScopeError CSimScopePlugin::reset(U32BIT s)
{
    ...
    if (e.isSuccess()) { m_devices[s].m_eShape = shape; m_devices[s].m_pLimits = p; }
    return e;
}
...
ScopeError CSimScopePlugin::getParameterRange(U32BIT s, U32BIT c, Enum_Scope_ParamId p, S_Scope_ParameterRange& o)
{
    const S_SimDevice* d = dev(s);
    if (d == nullptr) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
    ...
    case Enum_Scope_ParamId::m_enumVerticalPosition:
        o.m_dMin = -L->m_dVertOffsetMax; o.m_dMax = L->m_dVertOffsetMax; o.m_dResolution = 1.0e-3; break;
    case Enum_Scope_ParamId::m_enumTimebaseScale:
        o.m_dMin = L->m_dTimebaseMin; o.m_dMax = L->m_dTimebaseMax; o.m_dResolution = 1.0e-12; break;
    case Enum_Scope_ParamId::m_enumTimebasePosition:
        o.m_dMin = -L->m_dTimebaseMax * 10.0; o.m_dMax = L->m_dTimebaseMax * 10.0; break;
    ...
    case Enum_Scope_ParamId::m_enumMeasureThreshold:
        o.m_dMin = ScopeShared::MEASURE_THRESHOLD_MIN; o.m_dMax = ScopeShared::MEASURE_THRESHOLD_MAX; break;
    default:
...
ScopeError CSimScopePlugin::autoscale(U32BIT s)
{
    S_SimDevice* d = dev(s);
    if (d == nullptr) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
    for (int i = 0; i < d->m_vVertScale.size(); ++i) { d->m_vVertScale[i] = 0.1; d->m_vVertOffset[i] = 0.0; }
...
ScopeError CSimScopePlugin::enableChannel(U32BIT s, U32BIT c, bool v)
{
    S_SimDevice* d = dev(s);
    if (d == nullptr) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
    if (!validChannel(d, c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL,
...
ScopeError CSimScopePlugin::setVerticalScale(U32BIT s, U32BIT c, FDOUBLE v)
{
    ...
    if (v < d->m_pLimits->m_dVertScaleMin || v > d->m_pLimits->m_dVertScaleMax)
        return ScopeError(Enum_Scope_ErrorCode::PARAMETER_OUT_OF_RANGE,
            ...
            .arg(v).arg(d->m_pLimits->m_dVertScaleMin).arg(d->m_pLimits->m_dVertScaleMax));
...
ScopeError CSimScopePlugin::setTimebaseScale(U32BIT s, FDOUBLE v)
{
    ...
    if (v < d->m_pLimits->m_dTimebaseMin || v > d->m_pLimits->m_dTimebaseMax)
        return ScopeError(Enum_Scope_ErrorCode::PARAMETER_OUT_OF_RANGE,
            ...
            .arg(v).arg(d->m_pLimits->m_dTimebaseMin).arg(d->m_pLimits->m_dTimebaseMax));
    d->m_dTimebase = v;
    return ScopeError();
}
...
ScopeError CSimScopePlugin::getSampleRate(U32BIT s, FDOUBLE& o)
{
    ...
    if (o > d->m_pLimits->m_dMaxSampleRate) o = d->m_pLimits->m_dMaxSampleRate;
    return ScopeError();
}
ScopeError CSimScopePlugin::setMemoryDepth(U32BIT s, U32BIT v)
{
    ...
    if (v < 1 || v > d->m_pLimits->m_u32MaxMemoryDepth)
        return ScopeError(Enum_Scope_ErrorCode::PARAMETER_OUT_OF_RANGE,
            QStringLiteral("memory depth %1 out of range [1 .. %2]").arg(v).arg(d->m_pLimits->m_u32MaxMemoryDepth));
    d->m_u32MemDepth = v;
    return ScopeError();
}
...
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eTrigMode = v; return ScopeError(); }
ScopeError CSimScopePlugin::getTriggerMode(U32BIT s, Enum_Scope_TriggerMode& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eTrigMode; return ScopeError(); }
ScopeError CSimScopePlugin::setTriggerType(U32BIT s, Enum_Scope_TriggerType v)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eTrigType = v; return ScopeError(); }
ScopeError CSimScopePlugin::getTriggerType(U32BIT s, Enum_Scope_TriggerType& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eTrigType; return ScopeError(); }
ScopeError CSimScopePlugin::setTriggerSource(U32BIT s, Enum_Scope_TriggerSource v)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eTrigSource = v; return ScopeError(); }
ScopeError CSimScopePlugin::getTriggerSource(U32BIT s, Enum_Scope_TriggerSource& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eTrigSource; return ScopeError(); }
ScopeError CSimScopePlugin::setTriggerSlope(U32BIT s, Enum_Scope_TriggerSlope v)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eTrigSlope = v; return ScopeError(); }
ScopeError CSimScopePlugin::getTriggerSlope(U32BIT s, Enum_Scope_TriggerSlope& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eTrigSlope; return ScopeError(); }
...
{
    S_SimDevice* d = dev(s);
    if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
...
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_dTrigHoldoff; return ScopeError(); }
ScopeError CSimScopePlugin::getTriggerState(U32BIT s, Enum_Scope_TriggerState& o)
{
    S_SimDevice* d = dev(s);
    if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
    o = (d->m_eAcqState == Enum_Scope_AcqState::m_enumRunning)
            ? Enum_Scope_TriggerState::m_enumAuto : Enum_Scope_TriggerState::m_enumTriggered;
    return ScopeError();
}
...
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eAcqMode = v; return ScopeError(); }
ScopeError CSimScopePlugin::getAcqMode(U32BIT s, Enum_Scope_AcqMode& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eAcqMode; return ScopeError(); }
ScopeError CSimScopePlugin::setAverageCount(U32BIT s, U32BIT v)
{
    ...
    if (v < 1 || static_cast<int>(v) > d->m_pLimits->m_iAvgCountMax)
        return ScopeError(Enum_Scope_ErrorCode::PARAMETER_OUT_OF_RANGE,
            QStringLiteral("average count %1 out of range [1 .. %2]").arg(v).arg(d->m_pLimits->m_iAvgCountMax));
    d->m_iAvgCount = static_cast<int>(v);
    return ScopeError();
}
ScopeError CSimScopePlugin::getAverageCount(U32BIT s, U32BIT& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = static_cast<U32BIT>(d->m_iAvgCount); return ScopeError(); }
ScopeError CSimScopePlugin::getAcquisitionState(U32BIT s, Enum_Scope_AcqState& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eAcqState; return ScopeError(); }

/*============================================================================
 *  Waveform
 *==========================================================================*/
ScopeError CSimScopePlugin::setWaveformSource(U32BIT s, Enum_Scope_WaveformSource v, U32BIT i)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eWfmSource = v; d->m_iWfmSourceIndex = static_cast<int>(i); return ScopeError(); }
ScopeError CSimScopePlugin::getWaveformSource(U32BIT s, Enum_Scope_WaveformSource& o, U32BIT& i)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eWfmSource; i = static_cast<U32BIT>(d->m_iWfmSourceIndex); return ScopeError(); }
ScopeError CSimScopePlugin::setWaveformFormat(U32BIT s, Enum_Scope_WaveformFormat v)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); d->m_eWfmFormat = v; return ScopeError(); }
ScopeError CSimScopePlugin::getWaveformFormat(U32BIT s, Enum_Scope_WaveformFormat& o)
{ S_SimDevice* d = dev(s); if (!d) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = d->m_eWfmFormat; return ScopeError(); }
...
void CSimScopePlugin::synthesize(const S_SimDevice* d, U32BIT c,
                                 QVector<FDOUBLE>& out_time, QVector<FDOUBLE>& out_volts) const
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (d->m_vCoupling.value(c - 1) == Enum_Scope_Coupling::m_enumGND) v = 0.0;
...
ScopeError CSimScopePlugin::getWaveformPreamble(U32BIT s, U32BIT c, S_Scope_WaveformPreamble& o)
{
    ...
    o.m_iFormat = (d->m_eWfmFormat == Enum_Scope_WaveformFormat::m_enumByte) ? 0
                : (d->m_eWfmFormat == Enum_Scope_WaveformFormat::m_enumWord) ? 1 : 4;
 ...
 *==========================================================================*/
double CSimScopePlugin::computeMeasurement(const S_SimDevice* d, U32BIT c, Enum_Scope_MeasType t, bool& out_bValid) const
{
    out_bValid = true;
    QVector<FDOUBLE> time, volts;
    synthesize(d, c, time, volts);
    const int n = volts.size();
    if (n < 2) { out_bValid = false; return 0.0; }
    ...
    switch (t) {
    case Enum_Scope_MeasType::m_enumVpp:       return vmax - vmin;
    ...
    case Enum_Scope_MeasType::m_enumVavg:      return vavg;
    case Enum_Scope_MeasType::m_enumFrequency: return freq;
    case Enum_Scope_MeasType::m_enumPeriod:    return period;
    case Enum_Scope_MeasType::m_enumRiseTime:  return period * 0.1;
    case Enum_Scope_MeasType::m_enumFallTime:  return period * 0.1;
    case Enum_Scope_MeasType::m_enumPosWidth:  return period * 0.5;
    case Enum_Scope_MeasType::m_enumNegWidth:  return period * 0.5;
    case Enum_Scope_MeasType::m_enumDutyCycle: return 50.0;
    case Enum_Scope_MeasType::m_enumOvershoot: return 0.0;
    case Enum_Scope_MeasType::m_enumPhase:     return 0.0;
    case Enum_Scope_MeasType::m_enumDelay:     return 0.0;
    default: out_bValid = false; return 0.0;
    }
}
...
ScopeError CSimScopePlugin::readMeasurement(U32BIT s, U32BIT c, Enum_Scope_MeasType t, S_Scope_MeasurementResult& o)
{
    S_SimDevice* d = dev(s);
    if (!validChannel(d, c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    bool valid = false;
    ...
    o.m_dStdDev = 0.0;
    switch (t) {
    case Enum_Scope_MeasType::m_enumFrequency: o.m_strUnits = QStringLiteral("Hz"); break;
    ...
    case Enum_Scope_MeasType::m_enumNegWidth:  o.m_strUnits = QStringLiteral("s"); break;
    case Enum_Scope_MeasType::m_enumDutyCycle: o.m_strUnits = QStringLiteral("%"); break;
    case Enum_Scope_MeasType::m_enumPhase:     o.m_strUnits = QStringLiteral("deg"); break;
    default:                                   o.m_strUnits = QStringLiteral("V"); break;
    }
...
{ if (!isConnected(s)) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); o = QStringLiteral("0,\"No error\""); return ScopeError(); }
ScopeError CSimScopePlugin::writeScpi(U32BIT s, const QString&)
{ if (!isConnected(s)) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED); return ScopeError(); }
