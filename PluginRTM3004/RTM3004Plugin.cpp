 ...
 *==========================================================================*/
CRTM3004Plugin::CRTM3004Plugin()
    : m_limits{ "RTM3004", "Rohde & Schwarz", "RTM3000", "RTM3004",
                ...
                "0x0AAD", "0x01D6" }
    , m_pLimits(&m_limits)
    , m_strModel(QStringLiteral("RTM3004"))
{
}
...
S_Scope_PluginInfo CRTM3004Plugin::getPluginInfo() const
{
    ...
    qstrncpy(s.m_szDescription, "Rohde & Schwarz RTM3004 oscilloscope, 1 GHz, 4 analog ch",
...
S_Scope_Capabilities CRTM3004Plugin::getCapabilities() const
{
 ...
 *==========================================================================*/
CRTM3004Plugin::S_DeviceInstance* CRTM3004Plugin::dev(U32BIT s)
{
    auto it = m_devices.find(s);
    return (it == m_devices.end()) ? nullptr : &it.value();
}
bool CRTM3004Plugin::validChannel(U32BIT c) const
{
    return c >= 1 && static_cast<int>(c) <= m_pLimits->m_iAnalogChannels;
}
QByteArray CRTM3004Plugin::fmtD(FDOUBLE v)
{
    char sz[40];
    std::snprintf(sz, sizeof(sz), "%.6G", v);
    return QByteArray(sz);
}
int CRTM3004Plugin::chanOf(Enum_Scope_TriggerSource e)
{
    switch (e) {
...
ScopeError CRTM3004Plugin::visaError(ViStatus st, const QString& ctx)
{
    if (st == VI_ERROR_TMO)
...
ScopeError CRTM3004Plugin::writeLine(U32BIT s, const QByteArray& cmd)
{
...
ScopeError CRTM3004Plugin::readLine(U32BIT s, QByteArray& resp)
{
    resp.clear();
    S_DeviceInstance* d = dev(s);
    if (d == nullptr || !d->m_bConnected) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
...
ScopeError CRTM3004Plugin::queryLine(U32BIT s, const QByteArray& cmd, QByteArray& resp)
{
...
ScopeError CRTM3004Plugin::readBinaryBlock(U32BIT s, QByteArray& payload)
{
    payload.clear();
    S_DeviceInstance* d = dev(s);
    if (d == nullptr || !d->m_bConnected) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
...
ScopeError CRTM3004Plugin::sendChecked(U32BIT s, const QByteArray& cmd)
{
    ScopeError e = writeLine(s, cmd);
    if (!e.isSuccess()) return e;
    QByteArray resp;
...
ScopeError CRTM3004Plugin::queryDouble(U32BIT s, const QByteArray& cmd, FDOUBLE& o)
{
    QByteArray resp;
...
ScopeError CRTM3004Plugin::setDouble(U32BIT s, const char* scpi, FDOUBLE v, FDOUBLE lo, FDOUBLE hi, const char* what)
{
    if (v < lo || v > hi)
 ...
 *  Connection
 *==========================================================================*/
ScopeError CRTM3004Plugin::connect(U32BIT s, const S_Scope_ConnectionConfig& c)
{
...
ScopeError CRTM3004Plugin::disconnect(U32BIT s)
{
    S_DeviceInstance* d = dev(s);
...
bool CRTM3004Plugin::isConnected(U32BIT s) const
{
    auto it = m_devices.find(s);
    return it != m_devices.end() && it.value().m_bConnected;
}

ScopeError CRTM3004Plugin::reset(U32BIT s)       { return sendChecked(s, QByteArrayLiteral("*RST")); }
ScopeError CRTM3004Plugin::clearStatus(U32BIT s) { return writeLine(s, QByteArrayLiteral("*CLS")); }

ScopeError CRTM3004Plugin::setTimeout(U32BIT s, U32BIT t)
{
    S_DeviceInstance* d = dev(s);
    if (d == nullptr) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
    d->m_u32Timeout = t;
...
ScopeError CRTM3004Plugin::getParameterRange(U32BIT s, U32BIT c, Enum_Scope_ParamId p, S_Scope_ParameterRange& o)
{
    if (!isConnected(s)) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
    ...
    case Enum_Scope_ParamId::m_enumAwgFrequency:
        if (!L->m_bHasAWG) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
        o.m_dMin = 0.1; o.m_dMax = L->m_dAwgFreqMax; break;
    case Enum_Scope_ParamId::m_enumAwgAmplitude:
        if (!L->m_bHasAWG) return ScopeError(Enum_Scope_ErrorCode::NOT_SUPPORTED);
        o.m_dMin = 0.0; o.m_dMax = L->m_dAwgAmplMax; break;
    default:
...
ScopeError CRTM3004Plugin::autoscale(U32BIT s)    { return sendChecked(s, QByteArrayLiteral("AUToscale")); }
ScopeError CRTM3004Plugin::run(U32BIT s)
{
    ScopeError e = writeLine(s, QByteArrayLiteral("RUN"));
    if (e.isSuccess()) { S_DeviceInstance* d = dev(s); if (d) d->m_bRunning = true; }
    return e;
}
ScopeError CRTM3004Plugin::stop(U32BIT s)
{
    ScopeError e = writeLine(s, QByteArrayLiteral("STOP"));
    if (e.isSuccess()) { S_DeviceInstance* d = dev(s); if (d) d->m_bRunning = false; }
    return e;
}
ScopeError CRTM3004Plugin::single(U32BIT s)
{
    ScopeError e = writeLine(s, QByteArrayLiteral("SINGle"));
    if (e.isSuccess()) { S_DeviceInstance* d = dev(s); if (d) d->m_bRunning = false; }
    return e;
}
ScopeError CRTM3004Plugin::forceTrigger(U32BIT s) { return writeLine(s, QByteArrayLiteral("TRIGger:FORCe")); }

/*============================================================================
 *  Vertical
 *==========================================================================*/
ScopeError CRTM3004Plugin::enableChannel(U32BIT s, U32BIT c, bool v)
{
...
ScopeError CRTM3004Plugin::isChannelEnabled(U32BIT s, U32BIT c, bool& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArray("CHANnel") + QByteArray::number(c) + ":STATe?", d);
    if (e.isSuccess()) o = (d != 0.0);
    return e;
}
ScopeError CRTM3004Plugin::setVerticalScale(U32BIT s, U32BIT c, FDOUBLE v)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return setDouble(s, (QByteArray("CHANnel") + QByteArray::number(c) + ":SCALe").constData(),
                     v, m_pLimits->m_dVertScaleMin, m_pLimits->m_dVertScaleMax, "vertical scale");
}
ScopeError CRTM3004Plugin::getVerticalScale(U32BIT s, U32BIT c, FDOUBLE& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return queryDouble(s, QByteArray("CHANnel") + QByteArray::number(c) + ":SCALe?", o);
}
ScopeError CRTM3004Plugin::setVerticalOffset(U32BIT s, U32BIT c, FDOUBLE v)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return setDouble(s, (QByteArray("CHANnel") + QByteArray::number(c) + ":OFFSet").constData(),
                     v, -m_pLimits->m_dVertOffsetMax, m_pLimits->m_dVertOffsetMax, "vertical offset");
}
ScopeError CRTM3004Plugin::getVerticalOffset(U32BIT s, U32BIT c, FDOUBLE& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return queryDouble(s, QByteArray("CHANnel") + QByteArray::number(c) + ":OFFSet?", o);
}
ScopeError CRTM3004Plugin::setVerticalPosition(U32BIT s, U32BIT c, FDOUBLE v)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return sendChecked(s, QByteArray("CHANnel") + QByteArray::number(c) + ":POSition " + fmtD(v));
}
ScopeError CRTM3004Plugin::getVerticalPosition(U32BIT s, U32BIT c, FDOUBLE& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return queryDouble(s, QByteArray("CHANnel") + QByteArray::number(c) + ":POSition?", o);
}
ScopeError CRTM3004Plugin::setCoupling(U32BIT s, U32BIT c, Enum_Scope_Coupling v)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    const char* tok = (v == Enum_Scope_Coupling::m_enumAC) ? "ACLimit"
                    : (v == Enum_Scope_Coupling::m_enumGND) ? "GND" : "DCLimit";
    return sendChecked(s, QByteArray("CHANnel") + QByteArray::number(c) + ":COUPling " + tok);
}
ScopeError CRTM3004Plugin::getCoupling(U32BIT s, U32BIT c, Enum_Scope_Coupling& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    QByteArray r; ScopeError e = queryLine(s, QByteArray("CHANnel") + QByteArray::number(c) + ":COUPling?", r);
    if (!e.isSuccess()) return e;
...
ScopeError CRTM3004Plugin::setProbeAttenuation(U32BIT s, U32BIT c, FDOUBLE v)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    bool found = false;
    for (int i = 0; i < ScopeShared::PROBE_ATTEN_COUNT; ++i)
    ...
    return sendChecked(s, QByteArray("PROBe") + QByteArray::number(c) + ":SETup:ATTenuation:MANual " + fmtD(v));
}
ScopeError CRTM3004Plugin::getProbeAttenuation(U32BIT s, U32BIT c, FDOUBLE& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
 ...
 *  Horizontal
 *==========================================================================*/
ScopeError CRTM3004Plugin::setTimebaseScale(U32BIT s, FDOUBLE v)
{
    return setDouble(s, "TIMebase:SCALe", v, m_pLimits->m_dTimebaseMin, m_pLimits->m_dTimebaseMax, "timebase scale");
}
ScopeError CRTM3004Plugin::getTimebaseScale(U32BIT s, FDOUBLE& o)
{ return queryDouble(s, QByteArrayLiteral("TIMebase:SCALe?"), o); }
ScopeError CRTM3004Plugin::setTimebasePosition(U32BIT s, FDOUBLE v)
{ return sendChecked(s, QByteArray("TIMebase:POSition ") + fmtD(v)); }
ScopeError CRTM3004Plugin::getTimebasePosition(U32BIT s, FDOUBLE& o)
{ return queryDouble(s, QByteArrayLiteral("TIMebase:POSition?"), o); }
ScopeError CRTM3004Plugin::getSampleRate(U32BIT s, FDOUBLE& o)
{
    FDOUBLE sr = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("ACQuire:SRATe?"), sr);
    if (!e.isSuccess()) return e;
    if (sr <= 0.0) {
...
ScopeError CRTM3004Plugin::setMemoryDepth(U32BIT s, U32BIT v)
{
...
ScopeError CRTM3004Plugin::getMemoryDepth(U32BIT s, U32BIT& o)
{
    FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("ACQuire:POINts?"), d);
    if (!e.isSuccess()) return e;
 ...
 *  Trigger
 *==========================================================================*/
ScopeError CRTM3004Plugin::setTriggerMode(U32BIT s, Enum_Scope_TriggerMode v)
{
...
ScopeError CRTM3004Plugin::getTriggerMode(U32BIT s, Enum_Scope_TriggerMode& o)
{
...
ScopeError CRTM3004Plugin::setTriggerSource(U32BIT s, Enum_Scope_TriggerSource v)
{
...
ScopeError CRTM3004Plugin::getTriggerSource(U32BIT s, Enum_Scope_TriggerSource& o)
{
    QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral("TRIGger:A:SOURce?"), r);
    if (!e.isSuccess()) return e;
    const QString t = QString::fromLatin1(r).trimmed().toUpper();
    ...
    else if (t.contains(QStringLiteral("EXT"))) o = Enum_Scope_TriggerSource::m_enumExt;
    else o = Enum_Scope_TriggerSource::m_enumCh1;
    return ScopeError();
}
ScopeError CRTM3004Plugin::setTriggerSlope(U32BIT s, Enum_Scope_TriggerSlope v)
{
    const char* tok = (v == Enum_Scope_TriggerSlope::m_enumFalling) ? "NEGative"
                    : (v == Enum_Scope_TriggerSlope::m_enumEither) ? "EITHer" : "POSitive";
    return sendChecked(s, QByteArray("TRIGger:A:EDGE:SLOPe ") + tok);
}
ScopeError CRTM3004Plugin::getTriggerSlope(U32BIT s, Enum_Scope_TriggerSlope& o)
{
...
ScopeError CRTM3004Plugin::setTriggerLevel(U32BIT s, U32BIT c, FDOUBLE v)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return sendChecked(s, QByteArray("TRIGger:A:LEVel") + QByteArray::number(c) + ":VALue " + fmtD(v));
}
ScopeError CRTM3004Plugin::getTriggerLevel(U32BIT s, U32BIT c, FDOUBLE& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return queryDouble(s, QByteArray("TRIGger:A:LEVel") + QByteArray::number(c) + ":VALue?", o);
}
ScopeError CRTM3004Plugin::setTriggerHoldoff(U32BIT s, FDOUBLE v)
{
    return setDouble(s, "TRIGger:A:HOLDoff:TIME", v, m_pLimits->m_dTrigHoldoffMin, m_pLimits->m_dTrigHoldoffMax, "holdoff");
}
ScopeError CRTM3004Plugin::getTriggerHoldoff(U32BIT s, FDOUBLE& o)
{ return queryDouble(s, QByteArrayLiteral("TRIGger:A:HOLDoff:TIME?"), o); }
ScopeError CRTM3004Plugin::getTriggerState(U32BIT s, Enum_Scope_TriggerState& o)
{
    QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral("TRIGger:STATe?"), r);
    if (!e.isSuccess()) return e;
    const QString t = QString::fromLatin1(r).trimmed().toUpper();
    if (t.contains(QStringLiteral("TRIG"))) o = Enum_Scope_TriggerState::m_enumTriggered;
 ...
 *  Acquisition
 *==========================================================================*/
ScopeError CRTM3004Plugin::setAcqMode(U32BIT s, Enum_Scope_AcqMode v)
{
    const char* tok = "SAMPle";
    switch (v) {
    case Enum_Scope_AcqMode::m_enumPeakDetect: tok = "PDETect"; break;
    case Enum_Scope_AcqMode::m_enumAverage:    tok = "AVERage"; break;
    case Enum_Scope_AcqMode::m_enumHiRes:      tok = "HRESolution"; break;
    case Enum_Scope_AcqMode::m_enumEnvelope:   tok = "ENVElope"; break;
    default: break;
    }
    return sendChecked(s, QByteArray("ACQuire:MODE ") + tok);
}
ScopeError CRTM3004Plugin::getAcqMode(U32BIT s, Enum_Scope_AcqMode& o)
{
    QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral("ACQuire:MODE?"), r);
    if (!e.isSuccess()) return e;
    const QString t = QString::fromLatin1(r).trimmed().toUpper();
    if (t.startsWith(QStringLiteral("PD")) || t.startsWith(QStringLiteral("PEAK"))) o = Enum_Scope_AcqMode::m_enumPeakDetect;
    else if (t.startsWith(QStringLiteral("AVER"))) o = Enum_Scope_AcqMode::m_enumAverage;
    else if (t.startsWith(QStringLiteral("HRES"))) o = Enum_Scope_AcqMode::m_enumHiRes;
    else if (t.startsWith(QStringLiteral("ENV"))) o = Enum_Scope_AcqMode::m_enumEnvelope;
    else o = Enum_Scope_AcqMode::m_enumSample;
    return ScopeError();
}
ScopeError CRTM3004Plugin::setAverageCount(U32BIT s, U32BIT v)
{
...
ScopeError CRTM3004Plugin::getAverageCount(U32BIT s, U32BIT& o)
{
    FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("ACQuire:AVERage:COUNt?"), d);
    if (e.isSuccess()) o = static_cast<U32BIT>(d);
    return e;
}
ScopeError CRTM3004Plugin::getAcquisitionState(U32BIT s, Enum_Scope_AcqState& o)
{
    S_DeviceInstance* d = dev(s);
    if (d == nullptr || !d->m_bConnected) return ScopeError(Enum_Scope_ErrorCode::NOT_CONNECTED);
 ...
 *  Waveform
 *==========================================================================*/
ScopeError CRTM3004Plugin::setWaveformSource(U32BIT s, Enum_Scope_WaveformSource v, U32BIT i)
{
...
ScopeError CRTM3004Plugin::setWaveformFormat(U32BIT s, Enum_Scope_WaveformFormat v)
{
...
ScopeError CRTM3004Plugin::setWaveformPoints(U32BIT s, U32BIT v)
{
    if (v < 1 || v > m_pLimits->m_u32MaxMemoryDepth)
        return ScopeError(Enum_Scope_ErrorCode::PARAMETER_OUT_OF_RANGE,
...
ScopeError CRTM3004Plugin::getWaveformPreamble(U32BIT s, U32BIT c, S_Scope_WaveformPreamble& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    QByteArray r;
...
ScopeError CRTM3004Plugin::readWaveform(U32BIT s, U32BIT c, S_Scope_Waveform& o)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
...
ScopeError CRTM3004Plugin::digitizeChannel(U32BIT s, U32BIT c)
{
    if (!validChannel(c)) return ScopeError(Enum_Scope_ErrorCode::INVALID_CHANNEL);
    return single(s);
}
 ...
 *  Status
 *==========================================================================*/
ScopeError CRTM3004Plugin::readErrorStatus(U32BIT s, U32BIT, S_Scope_DeviceErrorStatus& o)
{
...
ScopeError CRTM3004Plugin::clearErrorStatus(U32BIT s, U32BIT) { return writeLine(s, QByteArrayLiteral("*CLS")); }
ScopeError CRTM3004Plugin::queryErrorQueue(U32BIT s, QString& o)
{
    QByteArray r; ScopeError e = queryLine(s, QByteArrayLiteral("SYST:ERR?"), r);
    if (e.isSuccess()) o = QString::fromLatin1(r).trimmed();
    return e;
}
ScopeError CRTM3004Plugin::readStatusByte(U32BIT s, U32BIT& o)
{
    FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("*STB?"), d);
    if (e.isSuccess()) o = static_cast<U32BIT>(d);
    return e;
}
ScopeError CRTM3004Plugin::readStandardEventStatus(U32BIT s, U32BIT& o)
{
    FDOUBLE d = 0; ScopeError e = queryDouble(s, QByteArrayLiteral("*ESR?"), d);
    if (e.isSuccess()) o = static_cast<U32BIT>(d);
    return e;
}
ScopeError CRTM3004Plugin::writeScpi(U32BIT s, const QString& v) { return writeLine(s, v.toLatin1()); }
ScopeError CRTM3004Plugin::queryScpi(U32BIT s, const QString& v, QString& o)
{
