...
HorizontalTriggerTab::HorizontalTriggerTab(QWidget* parent) : QWidget(parent)
{
    ...
    m_pTimebase->setDecimals(12);
    m_pTimebase->setRange(1e-10, 1000.0);
    m_pTimebase->setValue(1e-6);
    ...
    m_pTrigSource = new QComboBox(tbox);
    m_pTrigSource->addItems(QStringList() << "CH1" << "CH2" << "CH3" << "CH4" << "EXT");
    m_pTrigSlope = new QComboBox(tbox);
    m_pTrigSlope->addItems(QStringList() << "Rising" << "Falling" << "Either");
    m_pTrigMode = new QComboBox(tbox);
    m_pTrigMode->addItems(QStringList() << "Auto" << "Normal");
    m_pTrigLevel = new QDoubleSpinBox(tbox);
    m_pTrigLevel->setDecimals(4);
    m_pTrigLevel->setRange(-100.0, 100.0);
    m_pTrigLevel->setSuffix(tr(" V"));
...
void HorizontalTriggerTab::onApply()
{
    ...
    if (e.isSuccess())
    {
        e = mgr.setTriggerSource(TESTER_SCOPE,
                                 static_cast<Enum_Scope_TriggerSource>(m_pTrigSource->currentIndex()));
    }
    if (e.isSuccess())
    {
        e = mgr.setTriggerSlope(TESTER_SCOPE,
                                static_cast<Enum_Scope_TriggerSlope>(m_pTrigSlope->currentIndex()));
    }
    if (e.isSuccess())
    {
    ...
    if (e.isSuccess())
    {
        e = mgr.setTriggerLevel(TESTER_SCOPE, TESTER_CHANNEL, m_pTrigLevel->value());
    }
...
void HorizontalTriggerTab::setConnected(bool in_bConnected)
{
    ...
    if (in_bConnected)
    {
        ...
        if (CScopeManager::instance()
                .getParameterRange(TESTER_SCOPE, TESTER_CHANNEL, Enum_Scope_ParamId::m_enumTimebaseScale, r)
                .isSuccess())
        {
