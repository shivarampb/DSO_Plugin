# RTM3004 - Rohde & Schwarz (RTM3000)

Place the following PDFs in this folder (see `../README.md` for the sourcing
recipe). They are **not shipped**.

1. **RTM3000 User Manual (Remote Control chapters)** - the SCPI reference (single source of truth).
2. **RTM3004 datasheet** - drives this model's `S_ScopeLimits` row.

Expected files (any filename is fine; these are suggestions):
- `RTM3004_ProgrammingManual.pdf`
- `RTM3004_Datasheet.pdf`

**Status:** Headline specs VERIFIED vs datasheet (1 GHz, 4 ch, 5 GS/s interleaved, 40 M/ch, 16 digital opt, built-in gen).
