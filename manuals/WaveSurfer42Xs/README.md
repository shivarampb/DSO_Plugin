# WaveSurfer42Xs - Teledyne LeCroy (WaveSurfer Xs)

Place the following PDFs in this folder (see `../README.md` for the sourcing
recipe). They are **not shipped**.

1. **MAUI / X-Stream Automation Command Reference Manual** - the SCPI reference (single source of truth).
2. **WaveSurfer42Xs datasheet** - drives this model's `S_ScopeLimits` row.

Expected files (any filename is fine; these are suggestions):
- `WaveSurfer42Xs_ProgrammingManual.pdf`
- `WaveSurfer42Xs_Datasheet.pdf`
